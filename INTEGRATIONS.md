...
**cognis-devbox** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `cognis-devbox` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
cognis-devbox ... --format json | cognis-connect emit --to sigma   # Sigma rules
cognis-devbox ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
cognis-devbox ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
cognis-devbox ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `cognis-devbox` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="cognis-devbox") for rec in records]   # records = your JSON output
